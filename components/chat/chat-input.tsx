...
export const ChatInput = ({ apiUrl, query, name, type }: ChatInputProps) => {
  ...
  const onAddEmoji = (emoji: string) => {
    ...
    let content = form.getValues("content").split("");

    content.splice(selectionStart, selectionEnd - selectionStart, emoji);

    let updatedContent = content.join("");
              ...
              <FormControl>
                <div className="relative p-4 pb-6">
                  ...
                  <div className="absolute top-7 right-8">
                    <EmojiPicker
                      onChange={(emoji: string) => onAddEmoji(emoji)}
                    />
                  </div>
                </div>
              </FormControl>
